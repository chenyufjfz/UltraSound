...
if {[file exists cic_inp.vo]} {
	set hdl_ext "v"
} else {
	puts "Warning: Could not find cic_inp.$ipfs_ext!"
}
...
# test bench settings
set_global_assignment -name EDA_DESIGN_INSTANCE_NAME cic_inp_inst -section_id tb
set_global_assignment -name EDA_TEST_BENCH_MODULE_NAME work.cic_inp_tb -section_id tb

# IPFS file
set_global_assignment -name EDA_IPFS_FILE "cic_inp.${ipfs_ext}" -section_id eda_simulation -library work

foreach i $input_files {
  set_global_assignment -name EDA_TEST_BENCH_FILE "$i" -section_id tb -library work
}
set_global_assignment -name EDA_TEST_BENCH_FILE "cic_inp_tb.${hdl_ext}" -section_id tb -library work
